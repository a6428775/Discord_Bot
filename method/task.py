...
class Task(Cog_Extension):
        ...
        async def time_task():
            ...
            while not self.bot.is_closed():
                ...
                if now_time in jdata['time'] and self.counter == 0:
                    self.counter = 1
                    await self.channel.send('時間到 開始執行')
    ...
    async def set_time(self,ctx,*time):
        ...
        jdata['time']=time
        with open(os.path.join(os.path.dirname((os.path.dirname(__file__))),'setting.json'),'w',encoding='utf8') as jfile:
...
async def sendpic(self, count:int):
    ...
    dt1 = datetime.utcnow().replace(tzinfo=timezone.utc)
    dt2 = dt1.astimezone(timezone(timedelta(hours=8))) # 轉換時區 -> 東八區
    await self.channel.send('阿臣臣報時 現在時間 : ' + dt2.strftime("%Y-%m-%d %H:%M:%S")) # 將時間轉換為 string
    ...
    with open(os.path.join(os.path.dirname((os.path.dirname(__file__))),'cookies_jar.json')) as f:
        cookies = json.load(f)
    ...
    #更新這個清單就好
    insAccList=['__mii04','janie.lin','pyoapple','cathrynli',
                'kiki_hsieh','asukakiraran','miyami176','umy510266',
                'ms_puiyi','alinitydivine','nana.un','vernalu49',
                'kizunasakura','angelakuo0504','yolo_ioo','yua_mikami',
                'woowoowu.qq','thisisshl','imfaithxo','yuchinjou',
                'inkyung97','czz_mz','iamlove0410','demi59487',
                '920924jasmine','jessie_199711','eyes198877',
                'catherine0113','niny1212_','liz___85','kuanting1003',
                'u.710','qun_04','miss_any','yi_______07','sallymee',
                'changyachuu','sunnyrayyxo','diorwynn','irenelin0413',
                'lillywhite','boygirl.god','m.a.m.a.s.i.t.a','tehhan',
                'kannachan__','155cm_____','xiaojie._.1218','36d_yy',
                'kitty770126','tokbaes']
    i=0
    while(i<count):
        #亂數取帳號
